...
//       sliInit[svc] = +(99 + i * 0.01).toFixed(2)
//       targetInit[svc] = +(99.1 + i * 0.01).toFixed(2)
//     })
//
//     setServiceSLIs(sliInit)
//     setServiceTargets(targetInit)
//   }, [])
//
//   // ✅ REALTIME UPDATE
//   useEffect(() => {
...
//         const updated = {}
//         for (let svc in prev) {
//           let change = (Math.random() > 0.5 ? 0.01 : 0.02)
//           change *= Math.random() > 0.5 ? 1 : -1
//
//           let next = +(prev[svc] + change).toFixed(2)
//           next = Math.max(98.8, Math.min(99.45, next))
//           updated[svc] = next
//         }
//         return updated
//       })
...
//
//       setErrorBudget(prev => {
//         let next = +(prev + (Math.random() > 0.5 ? 0.1 : -0.1)).toFixed(2)
...
//
//   // ✅ COMPUTED
//   const currentSLI = serviceSLIs[selectedService] ?? 99
//   const targetSLI = serviceTargets[selectedService] ?? 99
//
//   const overallSLI =
//     Object.values(serviceTargets).length
//       ? (
//           Object.values(serviceTargets).reduce((a, b) => a + b, 0) /
//           Object.values(serviceTargets).length
//         ).toFixed(2)
//       : '99.00'
//
...
//         {/* Target */}
//         <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}>
//           <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Target SLO</div>
//           <div style={{
//             fontSize: 20, fontWeight: 600,
//             fontFamily: 'var(--font-mono)',
...
//         {/* Error Budget */}
//         <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}>
//           <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Error Budget</div>
...
//         {/* ✅ Overall */}
//         <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 10, padding: 12 }}>
//           <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Overall SLO</div>
//           <div style={{
//             fontSize: 20, fontWeight: 600,
//             fontFamily: 'var(--font-mono)',
//             color: 'var(--accent-cyan)'
...
const COLORS = ['#00d4ff', '#00ff88', '#ff7a00', '#a855f7', '#ffd700', '#ff3355']
...
export default function MetricsChart({ liveMetrics }) {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const [m, s] = await Promise.all([api.getMetrics(), api.getLogStats(60)])
      setMetrics(m)
      setLogStats(s)
    } finally {
      setLoading(false)
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setErrorBudget(prev => {
        const next = +(prev + (Math.random() > 0.5 ? 0.1 : -0.1)).toFixed(2)
        return Math.max(0, Math.min(10, next))
      })
    }, 5000)
  ...
  return (
    <div style={{ padding: 20, overflowY: 'auto', height: '100%', width: '100%' }}>
      ...
      {/* Body */}
      {selectedApp === 'Payment-Infrastructure' ? (
        <>
          {/* SLI CARDS */}
          <div
            style={{
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Requests: {requests}</div>
            </div>
            ...
            >
              ...
              <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>Avg of all services</div>
            </div>
          </div>
        ...
        >
          Charts are currently configured for <strong>Payment-Infrastructure</strong>. Select it to view metrics.
        </div>
      )}
    </div>
  )
